...
module.exports = async (ctx) => {
    ...
    const response = await axios({
        ...
        url:
            url +
            `?m=QueryData&dbcode=hgyd&rowcode=zb&colcode=sj&wds=%5B%5D&dfwds=%5B%7B%22wdcode%22%3A%22zb%22%2C%22valuecode%22%3A%22${sourceid}%22%7D%2C%7B%22wdcode%22%3A%22sj%22%2C%22valuecode%22%3A%222018-%2Clast120%22%7D%5D&k1=${now}`,
        headers: {
    ...
    const result = nodes.map((node) => ({
            ...
            unit: node.unit,
            desc: node.exp,
            data: datap.filter((p) => p.wds[0].valuecode === node.code),
        }));
