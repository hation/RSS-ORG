...
const options = {
    ...
    patterns: [new RegExp(/title_arrayELMT1513417890535948\((.+)\);/g), new RegExp(/title_arrayELMT1513417869206945\((.+)\);/g), new RegExp(/title_arrayELMT1513417907763951\((.+)\);/g)],
    mapper: (e) => {
        const items = e.split(/',?'?/).filter((e) => e.length > 0);
        return {
            title: items[1],
            link: items[0],
            description: items[1],
